function results = ClusteringEvaluationMetrics(groundtrue,predict)

predict = bestMap(groundtrue,predict);
Acc = computeAcc(predict,groundtrue);

MIhat = MutualInfo(predict,groundtrue);
Purity = computePurity(predict,groundtrue);
ari = ARI(predict,groundtrue);
recall = Recall(groundtrue,predict);
fscore = Fscore(groundtrue,predict);
Entropy = entropy(groundtrue,predict);

% predict = bestMap(groundtrue,predict);
% Acc = computeAcc(predict,groundtrue);

results = [Acc, MIhat, Purity, ari, recall, fscore, Entropy];


function acc = computeAcc(L1,L2)

acc = sum(L1==L2)/length(L1);
...
function [p,dist]=computePurity(indCluster,classes)
...
function r = Recall(T,H)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
% T ground truth
% H result
% WHERE: f:f-score; r:recall
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
function [en,dist]=entropy(classes,indCluster)
...
numSample=numel(classes);                       %n = numel(A) returns the number of elements, n, in array A, equivalent to prod(size(A)).
uniClass=unique(classes);                       %C = unique(A) returns the same data as in A, but with no repetitions. C is in sorted order.
numClass=numel(uniClass);
uniCluster=unique(indCluster);
numCluster=numel(uniCluster);
dist=zeros(numCluster,numClass);
